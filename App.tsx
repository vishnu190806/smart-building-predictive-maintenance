...
function mapScoreToRisk(modelMode: ModelMode, score: number | null): number {
  ...
  const normalized = (0.5 - clamped) / 1.0; // 0.5 -> 0 risk, -0.5 -> 100% risk
  return Math.round(normalized * 100);
}

function getInsight(isAnomaly: boolean | null, risk: number): string {
  if (isAnomaly === null) return "Awaiting analysis. Adjust telemetry and run a health scan.";
  if (!isAnomaly && risk < 30) return "Operating within normal envelope. No immediate maintenance required.";
  if (!isAnomaly && risk < 60) return "Slight deviation detected. Consider scheduling routine inspection soon.";
...
const HealthMetricsStrip: React.FC<HealthMetricsStripProps> = ({ isAnomaly, riskPercent, lastCheckedISO }) => {
  ...
  return (
    <div className="metrics-strip glass-panel-compact">
      <div className="metric-item">
        ...
        <span className="metric-value status-pill-text" style={{ 
          color: statusColor,
      ...
      <div className="metric-item">
        ...
        <span className="metric-value time-stamp">{formatDisplayTime(lastCheckedISO)}</span>
      </div>
    </div>
  );
};
...
const SystemInsight: React.FC<SystemInsightProps> = ({ isAnomaly, riskPercent }) => {
  if (isAnomaly === null) return null;
  ...
  return (
    <div className={`system-insight-block ${isHighRisk ? 'warning-insight' : 'info-insight'}`}>
  ...
  return (
    <div className={`model-tooltip ${mode}`}>
      <h4 className="tooltip-title">
        {isUnsupervised ? "Anomaly Mode – IsolationForest" : "Failure Mode – RandomForest"}
      </h4>
    ...
    >
      ...
      <div className="toggle-wrapper">
        <button
          ...
          className={`model-toggle ${isSupervised ? "supervised" : "unsupervised"}`}
        >
          <div className="toggle-track">
            <span className={`toggle-label left ${!isSupervised ? "active" : ""}`}>Anomaly</span>
            <span className={`toggle-label right ${isSupervised ? "active" : ""}`}>Failure</span>
...
const App: React.FC = () => {
  ...
  const updateActiveSensor = (key: keyof Pick<Asset, 'rotationalSpeed' | 'processTemperature' | 'torque' | 'toolWear'>, value: number) => {
    setAssets(prev => prev.map(asset => 
      asset.id === activeAssetId ? { ...asset, [key]: value } : asset
    ));
  };

  const handleAnalyze = useCallback(async (event: React.MouseEvent<HTMLButtonElement>) => {
    if (isLoading) return;
    ...
    try {
      const modeQuery = modelMode === "unsupervised" ? "unsupervised" : "supervised";
      const response = await fetch(`${API_BASE_URL}/predict?mode=${modeQuery}`, {
        method: "POST",
      ...
      setAssets(prev => prev.map(a => 
        a.id === activeAssetId 
          ? { 
              ...
              failureProbability: modelMode === "supervised" ? resultScore : a.failureProbability,
              lastChecked: timeStr,
  ...
  useEffect(() => {
    ...
    const currentAssetHistory = anomalyHistory.filter(h => h.assetId === activeAssetId);
    ...
    const getX = (i: number) => pad + (i / (currentAssetHistory.length - 1)) * (w - pad * 2);
    const getY = (val: number) => h - pad - ((val - minVal) / (range || 1)) * (h - pad * 2);
  ...
  const currentRisk = useMemo(() => mapScoreToRisk(modelMode, activeAsset.anomalyScore), [modelMode, activeAsset.anomalyScore]);
        ...
        <header>
          <div className="logo-section">
            <div className="logo-icon">
              <svg className="logo-glyph" viewBox="0 0 24 24" aria-hidden="true">
                <path d="M4 11.5L12 4l8 7.5v7a1.5 1.5 0 0 1-1.5 1.5H5.5A1.5 1.5 0 0 1 4 18.5v-7z" fill="none" stroke="white" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </div>
          ...
          <div className="header-actions">
            ...
            <div className={`api-pill ${apiStatus}`}>
              ...
              <span>{apiStatus === "connected" ? "SYSTEM CONNECTED" : "DISCONNECTED"}</span>
            </div>
          </div>
        </header>
        ...
        <main className="dashboard-grid">
          <section className="left-panel">
            <div className="glass-card">
              <div className="label-row" style={{ marginBottom: '2rem' }}>
                <div>
                  <h3 style={{ fontSize: '1.2rem', fontWeight: 700 }}>{activeAsset.name}</h3>
                  <p style={{ fontSize: '0.7rem', color: 'var(--text-secondary)', marginTop: '4px' }}>AI4I Advanced Telemetry</p>
                </div>
                <span className="probe-badge-v2">{activeAsset.probeId}</span>
              </div>

              {[
                { label: 'Rotational speed (rpm)', key: 'rotationalSpeed' as const, min: 800, max: 3000, step: 50 },
                { label: 'Process temperature (K)', key: 'processTemperature' as const, min: 290, max: 360, step: 1 },
                { label: 'Torque (Nm)', key: 'torque' as const, min: 0, max: 100, step: 1 },
                { label: 'Component wear (min)', key: 'toolWear' as const, min: 0, max: 300, step: 5 }
              ].map(sensor => (
                <div className="input-group" key={sensor.key}>
                  <div className="label-row">
                    <span className="label-text">{sensor.label}</span>
                    <span className="value-pill">{activeAsset[sensor.key]}</span>
                  </div>
                  <div className="slider-wrap">
                    <input 
                      type="range" min={sensor.min} max={sensor.max} step={sensor.step}
                      value={activeAsset[sensor.key]} 
                      onChange={(e) => updateActiveSensor(sensor.key, parseInt(e.target.value))} 
                    />
                  </div>
                </div>
              ))}

              <button className="btn-analyze" disabled={isLoading} onClick={handleAnalyze}>
                {isLoading ? <div className="loader"></div> : <span>ANALYZE {activeAsset.name.toUpperCase()}</span>}
              </button>
          ...
          <section className="right-panel">
            <div className="asset-stack">
              ...
              {assets.map((asset) => (
                <div key={asset.id} className={`asset-card clickable ${activeAssetId === asset.id ? 'active' : ''} ${activeAssetId === asset.id ? (assetGlow === 'green' ? 'glow-green' : assetGlow === 'red' ? 'glow-red' : '') : ''}`} onClick={() => setActiveAssetId(asset.id)}>
                  ...
                  <div className={`asset-status ${asset.isAnomaly === null ? '' : asset.isAnomaly ? 's-critical' : 's-healthy'}`}>
                    {asset.isAnomaly === null ? 'NOT CHECKED' : asset.isAnomaly ? 'ANOMALY' : 'HEALTHY'}
                  </div>
                </div>
              ))}
              <div className="chart-panel">
                <div className="chart-header">
                  <span className="chart-title">History: {activeAsset.name}</span>
                </div>
